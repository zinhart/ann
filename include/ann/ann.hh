...
namespace zinhart
{
	  ...
	  public:
		...
		template <class Precision_Type>
		  HOST std::int32_t forward_propagate_async(const bool & copy_device_to_host, 
								 ...
								 const std::uint32_t & ith_observation_index, const std::vector<LAYER_INFO> & total_layers,
								 const std::uint32_t & total_targets, const  Precision_Type * host_total_targets, 
								 const std::uint32_t & total_hidden_weights, const Precision_Type * host_total_hidden_weights,
								 const std::uint32_t & total_activations, Precision_Type * host_total_activations,
								 const Precision_Type * host_total_bias,
								 const Precision_Type * device_total_observations, const Precision_Type * device_total_hidden_weights, Precision_Type * device_total_activations
								)

		  {
			...
			for(ith_layer = 1; ith_layer < total_layers.size() - 1; ++ith_layer, ++current_layer, ++prior_layer)
			{
			  ...
			  // add in bias
			  if(call_axps_async(1.0, device_total_activations + current_activation_offset, host_total_bias[current_layer], total_layers[current_layer].second, stream) != 0)
				return 1;		

			  // call activation
			   call_activation(total_layers[current_layer].first, ACTIVATION_TYPE::OBJECTIVE, device_total_activations + current_activation_offset, total_layers[current_layer].second);
  ...
  /**/		
		...
		HOST void backward_propagate(const bool & copy_device_to_host, 
								 const cudaStream_t & stream, const cublasHandle_t & context, 
								 const std::uint32_t & ith_observation_index, const std::vector<LAYER_INFO> & total_layers,
								 const std::uint32_t & total_targets, const  Precision_Type * host_total_targets, 
								 const std::uint32_t & total_hidden_weights, const Precision_Type * host_total_hidden_weights,
