	                       ...
	                       double * host_total_activations_val, const double * host_total_bias_val, const double * host_total_hidden_weights_val)
{
  ...
  start = total_layers[1].second * thread_id;
  stop = total_layers[1].second * (thread_id + 1);
...
TEST(ffn_test, async_forward_propagate)
{
  ...
  std::uniform_int_distribution<std::uint32_t> case_dist(1,3);
  ...
  const std::uint32_t n_cuda_streams = 1/*MAX_CPU_THREADS*/;
  ...
  for(current_layer = 1, total_activations_length = 0; current_layer < total_layers.size(); ++current_layer )
	total_activations_length += total_layers[current_layer].second;// accumulate neurons in the hidden layers and output layet r
  ...
  //new
  total_activations_length *= n_cuda_streams;
  ...
  for(ith_case = 0; ith_case < total_cases; ++ith_case)
  {
	...
	for (ith_stream = 0; ith_stream < n_cuda_streams; ++ith_stream)
	{
	  ...
	  std::uint32_t start{ith_stream * total_layers[1].second * total_observations_length};
	  std::uint32_t stop{(ith_stream + 1) * total_layers[1].second* total_observations_length};
	  std::cout<<"(forward_prop)stream_id: "<<ith_stream<<" n_cuda_streams: "<<n_cuda_streams<<" start: "<<start<< " stop: "<<stop<<"\n";
	  ...
	  forward_propagate_tasks.push_back(zinhart::default_thread_pool::push_task(
													  [&]()
													  {
														...
														host_total_bias,
														device_total_observations, device_total_hidden_weights, device_total_activations + start);
                                                      }
						                                                      )
					                   );
	  ...
	  validation_tasks.push_back(zinhart::default_thread_pool::push_task(forward_propagate_cpu, 
																		 ...
																		 std::cref(total_layers),
																		 host_total_observations_val, host_total_targets_val, 
																		 host_total_activations_val + start, host_total_bias_val, host_total_hidden_weights_val 
																		)                                                     
		                        );
	}
  }
  ...
  for(ith_case = 0; ith_case < total_cases; ++ith_case)
  {
	...
	{   
	  ...
	  std::uint32_t start{ith_stream * total_layers[1].second* total_observations_length};
	  std::uint32_t stop{(ith_stream + 1) * total_layers[1].second* total_observations_length};
	  std::cout<<"stream_id: "<<ith_stream<<" n_cuda_streams: "<<n_cuda_streams<<" start: "<<start<< " stop: "<<stop<<"\n";
