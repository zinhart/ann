...
TEST(layer_test, identity_layer)
{
  ...
  // perform activation
  lyr->activate(objective(), layer_activations_ptr, lyr->get_size());
  ...
  lyr->activate(derivative(), layer_deltas_ptr, lyr->get_size());
...
TEST(layer_test, softmax_activation)
{
  ...
  // a softmax layer
  std::shared_ptr<layer<double>> lyr = std::make_shared<softmax_layer<double>>();
  lyr->set_size(neuron_dist(mt));
