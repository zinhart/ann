...
namespace zinhart
{
  ...
  template HOST std::pair<std::uint32_t, double *> get_total_observations(const ann<ffn> & model);
  template HOST std::pair<std::uint32_t, double *> get_total_hidden_weights(const ann<ffn> & model);
  template HOST std::pair<std::uint32_t, double *> get_total_activations(const ann<ffn> & model);
  template HOST std::pair<std::uint32_t, double *> get_total_error(const ann<ffn> & model);
  template HOST std::pair<std::uint32_t, double *> get_total_gradient(const ann<ffn> & model);
  template HOST std::pair<std::uint32_t, double *> get_total_deltas(const ann<ffn> & model);

  template HOST void add_layer(ann<ffn> & model, const LAYER_INFO & ith_layer);
  template HOST std::int32_t initialize_model(ann<ffn> & model,  
...
#endif
	  return model.init(total_observations, total_targets, device_id);
	  }
